...
pub fn save(repo_url: &str, branch: &str, commit_id: &str, env_files: &HashMap<String, String>) -> Result<(), String> {
    if let Some((owner, repo_name)) = shared::parse_github_repo(repo_url) {
        ...
        let commit = Commit { branch: branch.to_string(), env_files_paths };
